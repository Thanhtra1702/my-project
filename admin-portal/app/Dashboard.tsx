...
function TabVerticalButton({ active, onClick, label, icon }: any) { return (<div className="px-2 py-1"><button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3.5 text-[11px] font-bold uppercase tracking-widest transition-all rounded-sm border-2 ${active ? 'bg-[#007BFF] text-white shadow-md border-[#007BFF]' : 'text-slate-500 bg-transparent border-transparent hover:bg-blue-50/50 hover:border-[#007BFF]'}`}><div className={`shrink-0 ${active?'text-white':'text-slate-400'}`}>{icon}</div>{label}</button></div>); }
