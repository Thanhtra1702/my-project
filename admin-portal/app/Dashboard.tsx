...
function TabVerticalButton({ active, onClick, label, icon }: any) { return (<button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-4 text-[11px] font-bold uppercase tracking-widest transition-all border-b ${active ? 'bg-[#007BFF] text-white shadow-sm border-[#007BFF]' : 'text-slate-500 bg-transparent border-transparent hover:bg-slate-50 hover:border-slate-300'}`}><div className={`shrink-0 ${active?'text-white':'text-slate-400'}`}>{icon}</div>{label}</button>); }
