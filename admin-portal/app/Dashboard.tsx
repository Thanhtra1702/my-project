            ...
            <div className="space-y-6 animate-in fade-in duration-500">
              ...
              <div className="bg-white p-6 border border-slate-200 rounded-sm shadow-sm">
                <div className="flex justify-between items-end mb-4">
                  <div><h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Sử dụng tài nguyên</h3><p className="text-xl font-bold text-slate-900 mt-1">{Number(stats.total_tokens_all_time).toLocaleString()} <span className="text-xs text-slate-400 font-medium">/ {Number(tokenLimit).toLocaleString()}</span></p></div>
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Hiệu suất: {usagePercent.toFixed(1)}%</span>
                </div>
                <div className="w-full h-3 bg-slate-100 rounded-sm overflow-hidden border border-slate-200/50"><div className={`h-full transition-all duration-1000 ${usagePercent > 90 ? 'bg-rose-500' : 'bg-[#007BFF]'}`} style={{ width: `${usagePercent}%` }}></div></div>
              </div>
...
function TabVerticalButton({ active, onClick, label, icon }: any) { return (<button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 text-[11px] font-bold uppercase tracking-widest transition-all ${active ? 'bg-[#007BFF] text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}><div className={`shrink-0 ${active?'text-white':'text-slate-400'}`}>{icon}</div>{label}</button>); }
